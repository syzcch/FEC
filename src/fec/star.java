...
public class star {
    ...
    void STAR_decoding()
	{
		...
		int rs_data_nbr = 0; /*rs_data_nbr means the number of error in original data*/
		int rs_check_nbr = 0;/*rs_check_nbr  means the number of error in checksum data*/
		int rs_disk1 = -1;
		int rs_disk2 = -1;
		int rs_disk3 = -1;
			...
			if(rs_check_nbr == 0)
			{
				...
				if(restarts[data_disk_nbr] == 1)
				{
					...
					for( i = 0; i < block_nbr + 1; i++)
					{
						for( j = 0; j < block_size; j ++)
						{
							tmp_for_xor[i][j] = (char) (tmp[0][i * block_size + j] ^ tmp[0][((rs_disk2 - rs_disk1 + i) % p) * block_size + j]
					...
					/*    *tmp[0] = c[p][0] xor c[s][rs_disk2 - rs_disk1] 		  
					  ...
					  ...................................*/
					k = p - 1 - ( rs_disk2 - rs_disk1);/*c[p][k] xor c[p][p-1] = tmp_for_xor[k], and c[rs_disk2][p-1] is 0*/
					...
					while(0 != m)
					{
						...
						for( j = 0; j < block_size; j++)
						{
							check_data[data_disk_nbr][i * block_size + j] = (char) (tmp_for_xor[i][j] ^ check_data[data_disk_nbr][k * block_size + j]);
						}
						k = i;
						m--;
					}
	    ...
	    if(rs_data_nbr == 3)
		{
			...
			if( u == v)
			{
				for( i = 0; i < block_nbr + 1; i++)
				{
					for( j = 0; j < block_size; j ++)
					{
						tmp_for_xor[i][j] = (char) (tmp[0][i * block_size + j] ^ tmp[0][((t - r + i) % p) * block_size + j]
				...
				/*    *tmp_for_xor[0] = c[s][0] xor c[s][t-r] 
					  ...
					  *tmp_for_xor[i] = c[s][i] xor c[s][(t-r+i)%p]
					  ...................................*/
				k = p - 1 -( t - r );/*c[s][k] xor c[s][p-1] = tmp_for_xor[k], and c[s][p-1] is 0*/
				for( i = 0; i< block_size; i++)
				{
				...
				while(0 != m)
				{
					...
					for( j = 0; j < block_size; j++)
					{
						check_data[s][i * block_size + j] = (char) (tmp_for_xor[i][j] ^ check_data[s][k * block_size + j]);
					}
					k = i;
					m--;
				}
			...
			{	
				...
				/*now ,*tmp_for_xor[0] = c[s][u] xor c[s][p-u]  
				       *tmp_for_xor[i] = c[s][(u + i)%p] xor c[s][ ( p-u+i)%p ]
					   ...............................................*/
				i = u - 1;
				k = (u + i) % p;/*c[s][k] xor c[s][p-1] = tmp_for_xor[k], and c[s][p-1] is 0*/
				for( j = 0; j< block_size; j++)
				{
				...
				while(0 != m)
				{
					...
					for( j = 0; j < block_size; j++)
					{
						check_data[s][((u + i) % p) * block_size + j] = (char) (tmp_for_xor[i][j] ^ check_data[s][k * block_size + j]);
					}
					k = (u + i) % p;
					m--;
				}
    ...
    void Evenodd_decoding_1(int rs_disk1, int rs_disk2)
    {
    	...
    	if(rs_disk1 < data_disk_nbr && rs_disk2 == data_disk_nbr)
    	{
			...
			for( i = 0; i < block_size; i++)
			{
				if(stripe ==  p - 1)
					...
					tmp_for_s[i] = (char) (tmp[stripe * block_size + i] ^ check_data[data_disk_nbr + 2][stripe * block_size + i]);
			}
    	...
    	/*	for( i = 0; i < block_nbr; i++)
    		...
    		while(true)
    		{
    			...
    			for( i = 0; i < block_size; i++)
    			{
    				tmp_for_s[i] = (char) (check_data[rs_disk1][k * block_size + i] ^ tmp[stripe * block_size + i]);
    			}
    		}		
    	}
    }
    ...
    void Evenodd_decoding( int[] restarts)
    {
    		...
    		if( rs_disk2 == data_disk_nbr)
    		{
    			...
    			for( i = 0; i < block_size; i++)
    			{
    				if( stripe == p - 1)
    					...
    					tmp_for_s[i] = (char) (tmp[stripe * block_size + i] ^ check_data[data_disk_nbr + 1][stripe * block_size + i]);
    			}
    	...
    	if(rs_nbr == 2 && rs_disk1 < data_disk_nbr && rs_disk2 < data_disk_nbr)/*两个数据盘出错*/
    	{
    		...
    		while(true)
    		{
    			...
    			for( j = 0; j < block_size; j++)
    			{
    				tmp_for_s[j] = (char) (check_data[rs_disk1][k * block_size + j] ^ tmp[stripe * block_size + j]);
    			}
