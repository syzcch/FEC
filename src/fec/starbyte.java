...
//    int **check_data;
    ...
    private static final int TOLERENCE = 3; // STAR can protect data against 2 failures
...
//    	memmove( check_data[data_disk_nbr + 2], tmp, check_data_size * sizeof(int));
 //       memmove( check_data[data_disk_nbr + 2], tmp, stripe_unit_size * sizeof(char));
...
//	    if(rs_disk1 != -1)memset(check_data[rs_disk1], 0 ,stripe_unit_size*sizeof(char));
//		if(rs_disk2 != -1)memset(check_data[rs_disk2], 0 ,stripe_unit_size*sizeof(char));
//		if(rs_disk3 != -1)memset(check_data[rs_disk3], 0 ,stripe_unit_size*sizeof(char));
	    if(rs_disk1 != -1){
...
//		out.printf("rs_disks : %d %d %d\n", rs_disk1,rs_disk2,rs_disk3);
	...
	//				tmp_for_xor = (int **)malloc( sizeof(int *) * p);
					...
					for( i = 0; i < block_nbr + 1; i++)
					{
						for( j = 0; j < block_size; j ++)
						{
							tmp_for_xor[i][j] = (byte) (tmp[0][i * block_size + j] ^ tmp[0][((rs_disk2 - rs_disk1 + i) % p) * block_size + j]
					...
					/*    *tmp[0] = c[p][0] xor c[s][rs_disk2 - rs_disk1] 		  
					  ...
					  ...................................*/
					k = p - 1 - ( rs_disk2 - rs_disk1);/*c[p][k] xor c[p][p-1] = tmp_for_xor[k],而且c[rs_disk2][p-1]的值全为零*/
					...
					while(0 != m)
					{
						...
						for( j = 0; j < block_size; j++)
						{
							check_data[data_disk_nbr][i * block_size + j] = (byte) (tmp_for_xor[i][j] ^ check_data[data_disk_nbr][k * block_size + j]);
						}
						k = i;
						m--;
					}
	...
	//		for( i = 0; i < check_data_size; i++)
			...
			if( u == v)/*处理三个错误的数据盘对称的情况*/
			{
				for( i = 0; i < block_nbr + 1; i++)
				{
					for( j = 0; j < block_size; j ++)
					{
						tmp_for_xor[i][j] = (byte) (tmp[0][i * block_size + j] ^ tmp[0][((t - r + i) % p) * block_size + j]
				...
				/*    *tmp_for_xor[0] = c[s][0] xor c[s][t-r] 
					  ...
					  *tmp_for_xor[i] = c[s][i] xor c[s][(t-r+i)%p]
					  ...................................*/
				k = p - 1 -( t - r );/*c[s][k] xor c[s][p-1] = tmp_for_xor[k],而且c[s][p-1]的值全为零*/
				for( i = 0; i< block_size; i++)
				{
				...
				while(0 != m)
				{
					...
					for( j = 0; j < block_size; j++)
					{
						check_data[s][i * block_size + j] = (byte) (tmp_for_xor[i][j] ^ check_data[s][k * block_size + j]);
					}
					k = i;
					m--;
				}
	...
	//			memset(count, 0, sizeof(int)*p);
				...
				while(0 != m)
				{
					...
					for( j = 0; j < block_size; j++)
					{
						check_data[s][((u + i) % p) * block_size + j] = (byte) (tmp_for_xor[i][j] ^ check_data[s][k * block_size + j]);
					}
					k = (u + i) % p;
					m--;
				}
    	...
    	if(rs_disk1 < data_disk_nbr && rs_disk2 == data_disk_nbr)/*一个数据盘错+行校验盘错*/
    	{
			...
			for( i = 0; i < block_size; i++)
			{
				if(stripe ==  p - 1)
					...
					tmp_for_s[i] = (byte) (tmp[stripe * block_size + i] ^ check_data[data_disk_nbr + 2][stripe * block_size + i]);
			}
...
//					memmove( check_data[rs_disk1] + j * block_size, tmp + i * block_size, block_size * sizeof(char));
					System.arraycopy(tmp, i * block_size, check_data[rs_disk1] , j * block_size, block_size);
				}
			}
			STAR_encoding_row();
	    }
...
//    		memmove( tmp_for_s, tmp + stripe * block_size, block_size* sizeof(int));/*由于后面用不到s了,所以*tmp_for_s这里用来存放各种中间值*/
...
//    			memmove( check_data[rs_disk2] + k * block_size, tmp_for_s, block_size * sizeof(int));
 //               memmove( check_data[rs_disk2] + k * block_size, tmp_for_s, block_size * sizeof(char));
    			...
    			for( i = 0; i < block_size; i++)
    			{
    				tmp_for_s[i] = (byte) (check_data[rs_disk1][k * block_size + i] ^ tmp[stripe * block_size + i]);
    			}
    		}		
    	}
    }
...
//    		for( i = 0; i < check_data_size; i++) 
    	...
    	if(rs_nbr == 2 && rs_disk1 < data_disk_nbr && rs_disk2 >= data_disk_nbr)/*一个数据盘错+一个校验盘错的情况*/
    	{
    		if( rs_disk2 == data_disk_nbr + 1)/*错误的校验盘是斜率为1的对角线校验*/
    		{
//    			for(i = 0; i < check_data_size; i++)/*先用行校验恢复数据盘*/
                for(i = 0; i < stripe_unit_size; i++)/*先用行校验恢复数据盘*/
    				for(j = 0; j <= data_disk_nbr; j++)
    					if( j != rs_disk1)
    						check_data[rs_disk1][i] ^= check_data[j][i];
    			STAR_encoding_diag1();
...
//    			memset(tmp_for_s, 0, block_size *sizeof(int));
    			...
    			for( i = 0; i < block_size; i++)
    			{
    				if( stripe == p - 1)
    					...
    					tmp_for_s[i] = (byte) (tmp[stripe * block_size + i] ^ check_data[data_disk_nbr + 1][stripe * block_size + i]);
    			}
...
//    		memmove( tmp_for_s, tmp + stripe * block_size, block_size * sizeof(int));/*由于后面用不到s了,所以*tmp_for_s这里用来存放各种中间值*/
//    		memmove( tmp_for_s, tmp + stripe * block_size, block_size * sizeof(char));/*由于后面用不到s了,所以*tmp_for_s这里用来存放各种中间值*/
    		System.arraycopy(tmp, stripe * block_size, tmp_for_s, 0, block_size);	
    		while(true)
    		{
    			k = (stripe - rs_disk2 + p) % p;/*第rs_disk2个数据盘的第k块在第stripe个条纹*/
...
//    			memmove( check_data[rs_disk2] + k * block_size, tmp_for_s, block_size * sizeof(int));
 //               memmove( check_data[rs_disk2] + k * block_size, tmp_for_s, block_size * sizeof(char));
    			...
    			for( j = 0; j < block_size; j++)
    			{
    				tmp_for_s[j] = (byte) (check_data[rs_disk1][k * block_size + j] ^ tmp[stripe * block_size + j]);
    			}
